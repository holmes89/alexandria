port module Api exposing (Cred, LoginRequest, LoginResult, addServerError, application, decodeErrors, login, logout, storeCredWith, username, viewerChanges)
...
type LoginResult
    = LoginResult (Result Http.Error Cred)
...
login : LoginRequest -> Cmd LoginResult
login loginRequest =
    let
        auth =
            "Basic " ++ Base64.encode (String.trim loginRequest.username ++ ":" ++ String.trim loginRequest.password)
    in
    Http.request
        { body = Http.emptyBody
        , expect = Http.expectJson LoginResult credDecoder
